...
const Play = (props: NextPage) => {
    ...
    useEffect(() => {
        if (state.spellings[index])
            ...
            inputRefs.current = inputRefs.current.slice(0, state.spellings[index].word.length);
    }, [state.index]);
    ...
    return (
        <>
            ...
            <div className='absolute right-6 md:right-24 top-10'>
            </div>
            <div className='h-full flex items-center  flex-col -my-16 md:-my-20'>
                ...
                <div className='flex my-auto items-center justify-center flex-col gap-y-5'>
                    <div>
                        <button disabled={soundPlaying} onClick={() => {
                            playSound()
                    ...
                    </div>
                    {
                        spellings.length > 0 &&
                        <AnimateSharedLayout>
                            <motion.div layout className='w-full text-md  md:text-xl flex flex-row items-center justify-center  gap-x-3'>
                                {new Array(spellings[index].word.length).fill(0).map((el, i) => {
                                    const correct = spellings[index].input[i] === spellings[index].word[i]
                    ...
                    {(!buttonsVisible || done) &&
                        <AnimateSharedLayout>
                            <motion.div
                                ...
                                variants={container} layout className='flex text-md  md:text-2xl md:justify-center flex-row items-start justify-between'>
                                {spellings[index].word.split('').
                                    map((el) =>
                                        <motion.div
                                            className={`rounded-sm w-full md:w-16 md:h-12 text-center text-gray-300 p-2 `} variants={item}>
                                            {el}
                                        </motion.div>)}
                            </motion.div>
                        </AnimateSharedLayout>
