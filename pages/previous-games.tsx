...
const PreviousGames = () => {
    ...
    return (
        <div>
            <Header />
            <div className='capitalize text-2xl md:text-3xl text-center p-2'>previous games</div>

            <div>{state.length == 0 ? 'No Data' : ''}</div>
            {state.length > 0 && <AnimateSharedLayout>
                <motion.div layout className='flex flex-col gap-y-5 items-center justify-center w-full'>
                    {state.map((el, i) => {
                        ...
                        return <motion.div className={`w-full flex justify-between flex-row rounded-md md:w-1/2 p-4 bg-gradient-to-r ${grad}`}>
                            <div>
                                ...
                                {el.difficulty && el.difficulty as string !== '' && <div className='text-xl capitalize'>
                                    Difficulty: {el.difficulty}
                                </div>}
                                <Link href='/result' onClick={() => {
                                    setRes(el.data.words.map((w) => ({ done: true, input: w.input, word: w.actualWord })), el.difficulty || '', true)
